...
export async function GET(req: NextRequest) {
  ...
  return Response.json({ generate: gen.map(j=>({ id: j.id, name: j.name, data: j.data, state: j.asJSON().state })), scrape: scr.map(j=>({ id: j.id, name: j.name, data: j.data, state: j.asJSON().state })) });
}
